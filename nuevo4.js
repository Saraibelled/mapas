console.log('test 03 - fixed text element initialization and clone rendering');
...
function initTuOnceIdeal() {
  ...
  // FUNCIÓN ACTUALIZADA: Asegura la creación del span de texto y gestiona la visualización según corresponda
  function updateFooterLogo() {
    if (!footerLogo) return;

    // Asegurarnos de que el span para el texto del dominio siempre esté creado en el DOM
    ...
    const selectors = [
      '.v-f .v-log__i',          // Logo específico en el Footer corporativo
      '.v-h .v-log__i',          // Logo en cabecera Vocento clásica
      '.v-log__i',               // Selector general de clase de logo
      '.v-f .v-log img',
      '.v-h .v-log img',
      '.v-header-logo img',
      '.header__logo img',
    ...
    // 1. Buscar en el documento actual del iframe
    ...
    if (foundLogo) {
      ...
      if (src) {
        ...
        footerLogo.removeAttribute('data-no-logo');
    ...
    footerLogo.setAttribute('data-no-logo', 'true');
      ...
      try {
        ...
        const canvas = await window.html2canvas(systemContainer, {
          ...
          onclone: (clonedDocument) => {
            // Buscamos el contenedor clonado directamente
            ...
            if (clonedContainer) {
              ...
              if (clonedFooterLogo) {
                // Comprobamos si el logo es un SVG sin convertir (indica fallo de rasterización/CORS)
                const hasSvg = clonedFooterLogo.src && clonedFooterLogo.src.includes('.svg');
                const noLogo = hasSvg || clonedFooterLogo.getAttribute('data-no-logo') === 'true';

                if (!noLogo && clonedFooterLogo.src && !clonedFooterLogo.src.endsWith('/')) {
